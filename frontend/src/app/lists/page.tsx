  ...
  return (
    <div className="max-w-4xl mx-auto w-full space-y-8 animate-fade-in pb-12 pt-[136px] px-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {Object.entries(packing.categories).map(([catName, items]: [string, any], idx) => {
          const formattedName = catName.replace(/_/g, " ").replace(/\b\w/g, c => c.toUpperCase());
          ...
          return (
            <div key={idx} className="bg-white rounded-[24px] p-6 shadow-sm border border-gray-100 flex flex-col">
              <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2 border-b border-gray-100 pb-3">
                {catName === "clothing" && <span className="material-symbols-outlined text-orange-500">checkroom</span>}
                {catName === "toiletries" && <span className="material-symbols-outlined text-blue-500">soap</span>}
                {catName === "electronics" && <span className="material-symbols-outlined text-gray-800">devices</span>}
                {catName === "weather_specific" && <span className="material-symbols-outlined text-red-500">ac_unit</span>}
                {formattedName}
              </h3>
                  ...
                  <label key={itemIdx} className="flex items-start gap-3 cursor-pointer group hover:bg-gray-50 p-2 -mx-2 rounded-lg transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-gray-900 group-hover:text-[#E67E22] transition-colors peer-checked:line-through peer-checked:text-gray-400">
                        {item.name || item.item}
                        {item.quantity && <span className="ml-2 text-xs bg-gray-100 px-1.5 py-0.5 rounded text-gray-600 not-italic no-underline">x{item.quantity}</span>}
                      </p>
                      {item.reason && (
                        <p className="text-xs text-gray-500 opacity-80 mt-0.5 peer-checked:line-through truncate">{item.reason}</p>
                      )}
                    </div>
                  </label>
                )) : null}
